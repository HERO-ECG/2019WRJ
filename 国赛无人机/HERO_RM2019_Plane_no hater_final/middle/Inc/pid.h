...
typedef struct __Common_Pid_t{
	void (*CommonPidSet)(struct __Common_Pid_t *Common_Pid_t, uint32_t pid_mode,
										float max_output, float integral_uplimit, float integral_downlimit,
										float p, float i, float d);
	float (*CommonPidCalc)(struct __Common_Pid_t *pid, float get, float set);
	uint32_t pid_mode;
	float 	 max_output;				/*输出限幅*/
	float 	 integral_uplimit;			/*积分正向抗饱和*/
	float 	 integral_downlimit;		/*积分负向抗饱和*/
...
//extern Common_Pid_t power_pid[2];
...
extern float CommonPidCalc(Common_Pid_t *pid, float get, float set);
extern void CommonPidSet(Common_Pid_t *pid, uint32_t pid_mode,
										float max_output, float integral_uplimit, float integral_downlimit,
										float p, float i, float d);
